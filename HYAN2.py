...
if uploaded_file:
    ...
    if st.button("🚀 Start Clustering"):
        # Preprocess depending on if target_column is provided
        if target_column:
            train_clean = preprocess_df(train_df, target_column)
        else:
            train_clean = preprocess_unsupervised(train_df)

        features = [col for col in train_clean.columns if col != target_column] if target_column else train_clean.columns

        # Debug info for features
        st.write(f"Features selected ({len(features)}): {features}")
        st.write(f"Train clean shape before scaling: {train_clean.shape}")

        if len(features) == 0 or train_clean[features].empty:
