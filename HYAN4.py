...
if uploaded_file:
    ...
    if st.button("🚀 Start Clustering"):
        ...
        if len(features) == 0:
            fallback_cols = ['open', 'high', 'low', 'close', 'volume', 'VWAP']
            features = [col for col in fallback_cols if col in train_df.columns and col != target_column]
            if not features:
                st.error("🚫 No features available for clustering after preprocessing. Please check your dataset.")
                st.stop()
            train_clean = train_df[features + ([target_column] if target_column else [])].copy()
