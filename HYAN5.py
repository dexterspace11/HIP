...
st.title("🧠 Hybrid DNN-EQIC Clustering and Prediction (with PCA Dimensionality Reduction)")
...
if uploaded_file:
    ...
    n_components = st.slider("Number of PCA Components", 2, 10, 5) if use_pca else None

    if st.button("🚀 Start Clustering"):
        train_clean = preprocess_df(train_df, target_column)
        features = [col for col in train_clean.columns if col != target_column] if target_column else train_clean.columns.tolist()
        ...
        if not features:
            ...
            if use_pca and n_components < len(features):
            ...
            with tab1:
                ...
                proj = pca_viz.fit_transform(train_scaled)
                fig, ax = plt.subplots()
            ...
            st.dataframe(pd.DataFrame(centroids, columns=[f"PC{i+1}" for i in range(centroids.shape[1])]))
